...
<div class="container">
	...
	<div class="row">
		<div class="col-lg-12 text-left myPage_03_01-content-body">
			<h6 class="order_01-sub-title-page">
				<span class="order_01-sub-title">FAQ 관리</span>
			</h6>
		</div>
	</div>
	...
	<div class="row">
		...
		<div class="col-lg-2 offset-10 text-center adminUser_01-content-header">
			<input class="adminProduct_01-header-button adminProduct_01-button-top" type="button" value="FAQ 등록" onclick="add_notice()">
		</div>
	</div>

	<div class="row">
		<div class="col-lg-1 text-center order_01-content-header myPage_05-member-ranking-info adminUser_01-header-border">
			<h6 class="order_01-content-hedaer-text">FAQ 번호</h6>
		</div>
		<div class="col-lg-1 text-center order_01-content-header myPage_05-member-ranking-info adminUser_01-header-border">
		...
		<div class="col-lg-7 text-center order_01-content-header myPage_05-member-ranking-info adminUser_01-header-border">
			<h6 class="order_01-content-hedaer-text">FAQ 제목</h6>
		</div>
	...
	<c:if test="${empty itemList}">
		<br>
		<div class="col-lg-12 text-center">등록된 FAQ가 없습니다.</div>
	</c:if>
			...
			<c:if test="${not empty itemList[key].notice.notice_id && i< pageNoMax+1}">
				...
				<div class="row">
					...
					<div id="admin_notice_title${j}" class="col-lg-7 text-center admin_notice_01_list">
						<a href ="${contextPath}/admin/FAQ/FAQ_detail.do?notice_id=${itemList[key].notice.notice_id}" class="admin_notice_black">
					...
					<div class="col-lg-2 text-center admin_notice_01_list_btn_section">
						<input class="admin_notice_01_list_btn adminProduct_01-product-top" type="button" value="FAQ 수정" id="updateN_${j}" onclick="update_notice_form(this.id)"> 
						<input class="admin_notice_01_list_btn" id="deleteN_${j}" type="button" value="FAQ 삭제" onclick="delete_notice(this.id)">
					</div>
				</div>
...
<script type="text/javascript">
		...
		if(user_rank >= 3) {
			...
			if(confirmFlag){
				...
				$.ajax({
					type : "post",
					async : false,
					url : "${contextPath}/admin/FAQ/delete_FAQ.do",
					dataType : "text",
					data : {
						"notice_id" : notice_id			
					},
	...
	function update_notice_form(target) {
		...
		if(${userInfo.user_rank > 2}) {
			...
			location.href='${contextPath}/admin/FAQ/update_FAQ_form.do?notice_id='+notice_id;
	...
	function pageMove(no) {
		...
		if(no == "이전") {
			...
			document.location='${contextPath}/admin/FAQ/FAQ_list.do?pageNo='+(--getValue);
			}
		}
		else if (no == "다음") {
			...
			document.location='${contextPath}/admin/FAQ/FAQ_list.do?pageNo='+no;
		}
	}
	...
	function add_notice() {
		...
		if(user_rank >= 3) {
			document.location='${contextPath}/admin/FAQ/add_FAQ_form.do';
		}
		 else {
