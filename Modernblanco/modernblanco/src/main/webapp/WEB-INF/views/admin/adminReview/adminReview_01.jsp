...
<div class="container">
	...
	<div class="row">
		<div class="col-lg-12 text-left myPage_03_01-content-body">
			<h6 class="order_01-sub-title-page">
				<span class="order_01-sub-title">후기 관리</span>
			</h6>
		</div>
	</div>
	...
	<div class="row">
		...
		<div class="col-lg-3 text-center adminUser_01-content-header">
			조회 유형 <select class="adminUser_01-select-box-lookup"
				...
				<option value="noticeDate">작성 일자</option>
				<option value="productID">상품명</option>
		...
		<div class="col-lg-6 text-center adminUser_01-content-header">
			...
			<div id="search_option_product_box">
				검색할 키워드 : <input id="search_option_product"
	...
	<c:if test="${empty itemList}">
		<br>
		<div class="col-lg-12 text-center">등록된 후기가 없습니다.</div>
	</c:if>
		...
		<c:forEach var="i" begin="0" end="${itemSize}">
			...
			<c:if test="${not empty itemList[j] && i < pageNoMax}">
				...
				<div class="row">
					...
					<div class="col-lg-2 text-center admin_CS_QAList">
						${itemList[j].product_main_title}</div>
					...
					<c:if test="${itemList[j].notice_parent_no == 0 }">
						...
						<div class="col-lg-2 text-center admin_CS_QAList_oth">
							<input
								...
								id="deleteCS_${j}" type="button" value="후기 삭제"
					...
					<c:if test="${itemList[j].notice_parent_no == 1 }">
						...
						<div class="col-lg-2 text-center admin_CS_QAList">
							<input class="adminProduct_01-product" id="deleteCS_${j}"
								type="button" value="후기 삭제" onclick="update_state(this.id)">
						</div>
					</c:if>
				</div>
...
function pageMove(no) {
	...
	if(no == "이전") {
		...
		document.location='${contextPath}/admin/CS/review_list.do?pageNo='+(--getValue);
		}
	}
	else if (no == "다음") {
		...
		document.location='${contextPath}/admin/CS/review_list.do?pageNo='+no;
	}
}
...
function search_notice_to_option() {
	...
	if (searchOption == "noticeDate") {
		if(endDate == "" || beginDate == "") {
			...
			location.href='${contextPath}/admin/CS/review_list.do?search_option='+searchOption+'&search_value='+searchDate;
	...
	// 포함된 아이디 기준 조회
		...
		if (searchText.match(/\s/g)) {
			...
			location.href='${contextPath}/admin/CS/review_list.do?search_option='+searchOption+'&search_value='+searchText;
		}
	}
	
	// 주문 상태 기준 조회
	else if (searchOption == "state") {
			location.href='${contextPath}/admin/CS/review_list.do?search_option='+searchOption+'&search_value='+searchState;
	}

	// 전체 회원 조회
	else if (searchOption == "all") {
		location.href='${contextPath}/admin/CS/review_list.do';
	}
	
}

// 후기 삭제 및 답변 작성 Form 스크립트
function update_state(target) {
    ...
    if(update_option == "addCS") {
    	...
    	if(${userInfo.user_rank > 2}) {
        	...
        	submitFlag = confirm("해당 후기를 삭제하시겠습니까?");
